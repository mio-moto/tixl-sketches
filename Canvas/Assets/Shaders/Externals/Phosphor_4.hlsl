// from: https://fragcoord.xyz/s/pp740spv
...
float4 psMain(float4 fragCoord : SV_Position) : SV_Target
{
    ...
    for (float i = 0, z = 0, d = 0, s = 0; i++ < 8e1; fragColor += (cos(s + float4(0, 1, 2, 0)) + 1.0) / d * z)
    {
        float3 p = z * normalize(fragCoord.rgb * 2.0 - float3(Resolution, 1.0).xyy) * float3(1, -1, 1), a = normalize(
                   cos(float3(1, 2, 0) + Time - d * 8.0));
        p.z += 5.0, a = a * dot(a, p) - cross(a, p);
        for (d = 1.0; d++ < 9.0;)
        {
            a += sin(a * d + Time).yzx / d;
        }
        z += d = 0.05 * abs(length(p) - 3.0) + 0.04 * abs(s = a.y);
    }
    fragColor = tanh(fragColor / 3e4);
